//! Renders a read-out table once at the terminal's width, for a command that
//! prints and exits. Nothing here scrolls or holds state. The terminal already
//! scrolls, which keeps `tect graph | less` and `| grep` working.
...
/// Renders one read-out table as ANSI lines. `rows` holds cells in `header`'s
/// order, each with a flag saying that row reports a defect. That flag is the
/// only thing colour says.
...
/// Gives each table column its own content width where that fits. Where it does
/// not, the ratatui constraint solver decides the width.
...
/// Gives the narrowest terminal that folds no word mid-way. It sums each table
/// column at its longest single word, plus the gaps between columns and the
/// frame. Below this width a table stays lossless and reads badly, so the calling
/// command falls back to markdown.
pub fn floor(header: &[&str], rows: &[(Vec<String>, bool)]) -> usize {
...
/// Folds `text` to `room` at spaces. A word wider than the table column folds
/// mid-word. A table cell clips what it cannot draw, and a clipped module name is
/// a wrong answer.
pub fn wrap(text: &str, room: usize) -> Vec<String> {
...
/// Emits one escape sequence per run of buffer cells that share a style. Cells
/// that carry no style get none.
...
/// Leaves unset every style field the buffer cell does not set, so an unstyled run
/// carries no escape at all. Setting every style field would emit three resets.
...
mod tests {
    ...
    fn every_cell_is_drawn_inside_one_frame_at_the_width_asked_for() {
        ...
        // Table frame, header row, and one line per table row.
        assert_eq!(lines.len(), 5, "{drawn}");
        // As wide as the two table columns and no wider.
